...
# PLOT SELECT testsize AS x, FLOOR(bandwidth) AS y FROM test WHERE funcname='ScanWrite64PtrUnrollLoop' ORDER BY x
plot \
    'blah.txt' index 0 with linespoints

## PLOT SELECT testsize AS x, FLOOR(bandwidth) AS y FROM test
...
# indentation start
    ...
    ## PLOT SELECT testsize AS x, FLOOR(rate * 1e12) AS y FROM test
